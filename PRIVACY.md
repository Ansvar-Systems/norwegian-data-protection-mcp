...
When running locally via `npx @ansvar/norwegian-data-protection-mcp`, all database queries stay on your machine. The only external data flow is between the MCP client and the AI provider (e.g., Anthropic).
...
- **Tool responses**: Decision text, guidance content, topic data
...
- **General regulatory research**: Safe to use through any deployment -- Datatilsynet decisions and guidance are public information
- **Client-specific queries**: If your queries could reveal confidential business information (e.g., specific organisation compliance gaps), consider using the local npm package
- **Sensitive matters**: For confidential data protection compliance work, use the local deployment option
...
For privacy questions: https://github.com/Ansvar-Systems/norwegian-data-protection-mcp/issues
